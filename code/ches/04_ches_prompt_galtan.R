...
df <- readRDS("data/ches/tmp/03_ches_data_id_with_scores.rds")
...
for (run in 1:3) {
    ...
    for (current_country in unique(df$country)) {
        ...
        prompt <- paste0(
            "Parties can be classified in terms of their views on social and cultural values. ",
            "\"LIBERTARIAN\" or \"POSTMATERIALIST\" parties favor expanded personal freedoms, ",
            " for example, abortion rights, divorce, and same-sex marriage. ",
            "\"TRADITIONAL\" or \"AUTHORITARIAN\" parties reject these ideas in favor of ",
            "order, tradition, and stability, believing that government should be a firm moral authority ", 
            "on social and cultural issues. Where did political parties stand on LIBERTARIAN/TRADITIONAL issues? ",
            "Please analyze the following political parties and assign each a position score on an 11 points scale from 0 (Libertarian/postmaterialist) to 10 (Traditional/authoritarian) ",
        ...
        if (!is.null(chat_prompt)) {
            ...
            if (grepl("^\\{.*\\}$", clean_json)) {
                ...
                if (!is.null(parsed_json) && !is.null(parsed_json$party_positions)) {
                    ...
                    score_column <- paste0("galtan_", run)
            ...
            print(paste("Error with API for Run:", run, 
                       "Country:", current_country))
            # Log the error
            run_errors <- rbind(run_errors, 
                              data.frame(run = run,
...
# Save the results
saveRDS(df, "data/ches/tmp/04_ches_data_id_with_galtan.rds")
saveRDS(run_errors, "data/ches/tmp/scoring_errors_galtan.rds")
